...
@WebServlet("/changerecruiter.do")
public class ChangeRecruiterServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
	@Resource(name = "jdbc/onlinerecruiter")
    private DataSource ds;
	Connection conn;
	private ChangeRecruiterService userRegister= new ChangeRecruiterService();
	
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.getRequestDispatcher("view/changerecruiter.jsp").forward(request, response);	}
		...
		if (newpassword!=null && confirm_password!=null) {
			   ...
			   if (newpassword.equals(confirm_password)) {
				   try
					...
					{
						response.sendRedirect("changerecruiter.do");
